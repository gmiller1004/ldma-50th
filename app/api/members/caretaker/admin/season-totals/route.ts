...
/**
 * DELETE ?campSlug=&seasonFrom=&seasonTo=[&snapshotDate=]
 * Drops a single as-of column, or the whole season trend when snapshotDate is omitted.
 */
export async function DELETE(request: NextRequest) {
  ...
  const seasonFrom = request.nextUrl.searchParams.get("seasonFrom")?.trim() ?? "";
  const seasonTo = request.nextUrl.searchParams.get("seasonTo")?.trim() ?? "";
  const snapshotDateParam = request.nextUrl.searchParams.get("snapshotDate")?.trim() ?? "";
  ...
  if (snapshotDateParam && !isIsoDate(snapshotDateParam)) {
    return NextResponse.json({ error: "Valid snapshotDate required (YYYY-MM-DD)" }, { status: 400 });
  }

  try {
    const deleted = await deleteSeasonTotalSnapshots({
      campSlug,
      seasonFrom,
      seasonTo,
      snapshotDate: snapshotDateParam || null,
    });
    return NextResponse.json({ deleted }, { headers: { "Cache-Control": "no-store" } });
  } catch (e) {
    console.error("[season-totals DELETE]", e);
    return NextResponse.json({ error: "Failed to clear season totals" }, { status: 500 });
  }
}

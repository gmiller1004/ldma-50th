#!/usr/bin/env node
/**
 * Run the blog schema SQL against your Neon database.
 * Usage: node --env-file=.env.local scripts/init-blog-db.mjs
 */
...
const sqlPath = join(__dirname, "init-blog-db.sql");
...
async function run() {
  ...
  console.log("Running blog schema on Neon...");
  let ok = 0;
  let err = 0;
